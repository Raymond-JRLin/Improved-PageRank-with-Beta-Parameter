...
public class UnitSum {
    ...
    // we cannot time beta directly with sum, we should read PR(n - 1) from another mapper
    public static class BetaMapper extends Mapper<Object, Text, Text, DoubleWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        	...
        	String line = value.toString().trim();
            String[] toPr = line.split("\t");
            // split toPage and its subPr, and time beta to subPr
            double betaPr = Double.parseDouble(toPr[1]) * beta;
            context.write(new Text(toPr[0]), new DoubleWritable(betaPr));
        }
    }
